...
app.post('/api/orders/:id/confirm', auth, requireMaster, async (req, res) => {
  try {
    ...
    const address = STUDIO_ADDRESS
      ...
      : `\n📍 Адрес студии мастер пришлёт отдельно.`;
    await sendMessage(
      order.clientId,
      `✅ <b>Запись подтверждена!</b>${when}${address}\nМастер ждёт вас. Спасибо!`
    );
    // Памятка и противопоказания — для тату-услуг (не для консультации)
    if (order.serviceType !== 'consultation') {
      await sendMessage(order.clientId, MEMO_TEXT);
      await sendMessage(order.clientId, CONTRA_TEXT);
    }
    ok(res, order);
  } catch (e) {
    console.error(e);
